...
class Robot(Body):
    ...
    def __init__(self,
        ...
        if verbose:
            ...
            print('joint names:           ', len(
                self.jointNames()), self.jointNames())
            print('joint indexes:         ', len(
                self.jointIndexes()), self.jointIndexes())
            print('actuated joint names:  ', len(
                self.actuatedJointNames()), self.actuatedJointNames())
            print('actuated joint indexes:', len(
                self.actuatedJointIndexes()), self.actuatedJointIndexes())
            print('link names:            ', len(
                self.linkNames()), self.linkNames())
            print('link indexes:          ', len(
                self.linkIndexes()), self.linkIndexes())
        ...
        # build the mappings
        self._tau_map_pin_pb, self._q_map_pb_pin, self._q_map_pin_pb, self._v_map_pb_pin, self._v_map_pin_pb = self.computeBulletPinoccioMaps(
            self._model)
        ...
        self._T_root_w = pin.SE3(sim_utils.quaternionToMatrix(
            self._Q_root_w), self._x_root_w)
    ...
    def update(self):
        ...
        # update visuals
        if self._com_marker:
            self._sim.resetBasePose(
                self._com_marker, self.baseCoMPosition(), [1, 0, 0, 0])
    ...
    def resetJointState(self, q=None, v=None):
        ...
        if v is not None:
            self._sim.resetJointState(self.id(), self.actuatedJointIndexes(
            ), self._q_cmd[self._pos_idx_offset:], self._v_cmd[self._vel_idx_offset:])
            self.setActuatedJointPositions(self._q_cmd, self._v_cmd)
        else:
            self._sim.resetJointState(
                self.id(), self.actuatedJointIndexes(), self._q_cmd[self._pos_idx_offset:])
    ...
    def enablePositionControl(self):
        self._sim.enableJointPositionControlMode(
            self.id(), self.jointIndexes())
        self._controlMode = ControlMode.positionControl
        print(self._controlMode, 'enabled!')

    def enableTorqueControl(self):
        self._sim.enableJointTorqueControlMode(
            self.id(), self.jointIndexes(), 0)
    ...
    def setActuatedJointPositions(self, q, v=None):
        ...
        # set the command
        if v is not None:
            self._sim.jointPositionControl(self.id(), self.actuatedJointIndexes(
            ), self._q_cmd[self._pos_idx_offset:], self._v_cmd[self._vel_idx_offset:])
        else:
            self._sim.jointPositionControl(
                self.id(), self.actuatedJointIndexes(), self._q_cmd[self._pos_idx_offset:])
    ...
    def setActuatedJointTorques(self, tau):
        ...
        # set the torque
        self._sim.jointTorqueControl(
            self.id(), self.actuatedJointIndexes(), self._tau_cmd)
    ...
    def computeRootPose(self):
        ...
        x_b_w, Q_b_w = pb.multiplyTransforms(
            x_com_w, Q_com_w, x_b_com, Q_b_com)
    ...
    def updateState(self):
        ...
        if self._use_floating_base:
            ...
            v = np.concatenate(
                (R_b_w.dot(XP_com_w[:3]), R_b_w.dot(XP_com_w[3:]), v))
    ...
    def computeBulletPinoccioMaps(self, model):
        ...
        # position [x,Q,q]
        q_map_pb_pin = np.hstack((np.arange(self._pos_idx_offset), np.zeros(
            self.numActuatedJoints(), dtype=int)))
        q_map_pin_pb = np.hstack((np.arange(self._pos_idx_offset), np.zeros(
            self.numActuatedJoints(), dtype=int)))
        # velocity [v,omega,qdot]
        v_map_pb_pin = np.hstack((np.arange(self._vel_idx_offset), np.zeros(
            self.numActuatedJoints(), dtype=int)))
        v_map_pin_pb = np.hstack((np.arange(self._vel_idx_offset), np.zeros(
            self.numActuatedJoints(), dtype=int)))

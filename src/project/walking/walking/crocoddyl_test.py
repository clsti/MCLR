...
if WITHDISPLAY:
    ...
    while True:
        ...
        # Get final state from current solution
        x_final = solver.xs[-1]
        
        # Create new walking problem starting from final state
        new_solver = crocoddyl.SolverBoxDDP(
            gait.createWalkingProblem(
                x_final,
                walking_gait["stepLength"],
                walking_gait["stepHeight"],
        ...
        # Solve new problem
        xs_new = [x_final] * (new_solver.problem.T + 1)
        us_new = new_solver.problem.quasiStatic([x_final] * new_solver.problem.T)
        new_solver.solve(xs_new, us_new, 50, False, 0.1)
        
        # Update solver for next iteration
        solver = new_solver
        
        time.sleep(0.1)  # Small delay to control playback speed
